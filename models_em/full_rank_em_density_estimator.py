...
class FullRankEMDensityEstimator(nn.Module):
    def __init__(self, target_samples, K):
        ...
        self.Sigma = torch.cov(self.target_samples.T).unsqueeze(0).repeat(self.K, 1, 1)
    ...
    def compute_log_v(self, x):
        z = self.forward(x)
        unormalized_log_v = self.reference_log_density(z) + self.log_pi.unsqueeze(0).repeat(x.shape[0],
                                                                                            1) + self.log_det_J(x)
    ...
    def log_density(self, x):
        ...
        return torch.logsumexp(
            self.reference_log_density(z) + self.log_pi.unsqueeze(0).repeat(x.shape[0], 1) + self.log_det_J(x), dim=-1)
    ...
    def M_step(self, batch):
        ...
        temp = (batch.unsqueeze(1).repeat(1, self.K, 1) - self.m.unsqueeze(0).repeat(batch.shape[0], 1, 1)).unsqueeze(
            -1)
        self.Sigma = torch.sum(
            v.unsqueeze(-1).unsqueeze(-1).repeat(1, 1, self.p, self.p) * (temp @ torch.transpose(temp, -1, -2)),
